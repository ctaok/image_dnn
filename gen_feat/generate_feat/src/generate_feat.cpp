...
void GenerateFeat::init(int feat_dim, int label_dim,
    int max_sent_num, int max_sent_len,
    const char *work_directory, bool write_flag)
{
    _feat_dim     = feat_dim;
    _label_dim    = label_dim;
    _max_sent_num = max_sent_num;
    _max_sent_len = max_sent_len;
    strcpy(_work_directory, work_directory);

    char data_list_file[1024];
    sprintf(data_list_file, "%s/data.lst", _work_directory);
    if (write_flag) {
        _fp_data_lst = fopen(data_list_file, "wt");
        if (_fp_data_lst == NULL) {
            printf("Cannot open file %s to write!\n", data_list_file);
        }
    } else {
        _fp_data_lst = fopen(data_list_file, "rt");
        if (_fp_data_lst == NULL) {
    ...
    _feat_mat_vec.resize(_max_sent_num);
    if (_label_dim == 0) {
        _has_label = false;
    }
    else {
        _has_label = true;
        _label_vec_vec.resize(_max_sent_num);
    }
    _len_vec.resize(_max_sent_num);

    for (int ii = 0; ii < _max_sent_num; ii++) {
        _feat_mat_vec[ii].resize(_max_sent_len);
        for (int jj = 0; jj < _max_sent_len; jj++) {
            _feat_mat_vec[ii][jj].resize(_feat_dim);
        }
        if (_has_label) {
            _label_vec_vec[ii].resize(_max_sent_len);
        }
    }
}
                               ...
                               vector<int> &label_vec, int length)
{
    ...
    for (int ii = 0; ii < length; ii++) {
        ...
        for (int jj = 0; jj < _feat_dim; ++jj) {
            //printf("%f ", _feat_mat_vec[_sent_num][ii][jj]);
        }
        if (_has_label) {
            _label_vec_vec[_sent_num][ii] = label_vec[ii];
            //printf("%d\n", _label_vec_vec[_sent_num][ii]);
        }
    }
    //printf("\n");
...
void GenerateFeat::write(char *feat_file, char *label_file, char *desc_file)
{
    ...
    int sample_size = _feat_dim * sizeof(float);
    int sample_kind = 9;
    ...
    fwrite(&sample_size,   sizeof(int), 1, fp_feat);
    fwrite(&sample_kind,   sizeof(int), 1, fp_feat);
