...
int CLex::analysis_sent(const char* sen_str, int* wordid)
{
    int sign = 0;
    //bool flg = false;
    char *ptr  = NULL;
	char sen[1024];
	strcpy(sen, sen_str);
    char *temp = strtok_r(sen, "  ", &ptr);
    while (temp) {
        int idx = find_word(temp);
        //INFO("word, id = %s, %d\n", temp, idx);
        if (idx != -1) {
            wordid[sign] = idx;
        } else {
            wordid[sign] = _voc_size + 3; //unk word id = _voc_size+1
            //wordid[sign] = _voc_size; //unk word id = _voc_size+1
        }
...
int CLex::analysis_sent(const char* sen_str, int* wordid, vector<string> &unk_str_vec)
{
    int sign = 0;
    //bool flg = false;
    char *ptr  = NULL;
...
int CLex::remove_unk_words(int *word_ids, int word_num)
{
    int idx = 0;
    for (int ii = 0; ii < word_num; ii++) {
        if (word_ids[ii] == _unk_id) {
            continue;
        }
        word_ids[idx++] = word_ids[ii];
    }
    return idx;
}

string CLex::get_lexicon_by_id(int *word_ids, int word_num, vector<string> unk_str_vec, int &idx_unk)
{
	string out_str;
	for (int ii = 0; ii < word_num; ii++) {
		if (word_ids[ii] == _unk_id) {
			out_str.append(unk_str_vec[idx_unk++]);
		} else {
			out_str.append(get_lexicon_by_id(word_ids[ii]));
		}
	}
	return out_str;
}
